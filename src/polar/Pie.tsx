...
interface PieProps<DataPointType = any, DataValueType = any>
  ...
  /**
   * Whether labels outside the pie should be repositioned to avoid overlap.
   * This applies to Pie's default outside labels (label={true} and custom Pie label variants without `position`).
   *
   * @defaultValue false
   */
  ...
  minAngle?: number;
  /**
   * If the absolute angle of a pie sector is less than this value, its outside label and label line will not render.
   * This applies to Pie's default outside labels (label={true} and custom Pie label variants without `position`).
   *
   * @defaultValue 0
   */
...
const renderLabelItem = (option: PieLabel, props: PieLabelRenderProps, value: unknown) => {
  ...
  return (
    <Text {...props} verticalAnchor="middle" className={className}>
...
const getLabelGap = (fontSize: unknown): number => {
...
const resolveLabelCollisionsBySide = (
  labels: ReadonlyArray<PieComputedLabel>,
  side: PieSide,
  minGap: number,
  longRadialOffset: number,
): Array<PieComputedLabel> => {
  ...
  return sorted.map((label, index) => {
    ...
    // Stronger taper so the top tiny labels are visibly closer to the pie.
...
function PieLabels({
  ...
  const { label, labelLine, dataKey, minShowLabelAngle, avoidLabelOverlap } = props;
  ...
  const labelGap = getLabelGap(customLabelProps.fontSize);
  const offsetRadius =
    (typeof label === 'object' &&
      ...
      label.offsetRadius) ||
    20;
  const longRadialOffset = offsetRadius + 36;
  const rerouteSmallSliceAngleThreshold = 4;
  const reroutedLabelGap = labelGap + 4;
  const effectiveMinShowLabelAngle = Math.max(0, minShowLabelAngle ?? 0);

  const labelCandidates: Array<PieComputedLabel> = sectors
    .map((entry, index) => {
      const angle = Math.abs(entry.endAngle - entry.startAngle);
      if (angle < effectiveMinShowLabelAngle) {
        return null;
      }

      const midAngle = (entry.startAngle + entry.endAngle) / 2;
      const anchorPoint = polarToCartesian(entry.cx, entry.cy, entry.outerRadius, midAngle);
      const defaultLabelPoint = polarToCartesian(entry.cx, entry.cy, entry.outerRadius + offsetRadius, midAngle);
      const radialPoint = avoidLabelOverlap
        ? polarToCartesian(entry.cx, entry.cy, entry.outerRadius + longRadialOffset, midAngle)
        : defaultLabelPoint;
      const side: PieSide = defaultLabelPoint.x >= entry.cx ? 'right' : 'left';
      ...
      return {
        ...
        anchorPoint,
        radialPoint,
        bendPoint: defaultLabelPoint,
        labelPoint: defaultLabelPoint,
      };
    })
    .filter((item): item is PieComputedLabel => item != null);

  const labelsToRender = avoidLabelOverlap
    ? (() => {
        const labelsToReroute = labelCandidates.filter(
          label => label.sliceAngle <= rerouteSmallSliceAngleThreshold,
        );
        const labelsToKeepNatural = labelCandidates.filter(label => label.sliceAngle > rerouteSmallSliceAngleThreshold);

        const rerouted = [
          ...resolveLabelCollisionsBySide(labelsToReroute, 'left', reroutedLabelGap, longRadialOffset),
          ...resolveLabelCollisionsBySide(labelsToReroute, 'right', reroutedLabelGap, longRadialOffset),
        ].map(label => ({
          ...label,
          isRerouted: true,
        }));

        return [...labelsToKeepNatural, ...rerouted].sort((a, b) => a.index - b.index);
      })()
    : labelCandidates;

  const labels = labelsToRender.map(
    ({ index, entry, textAnchor, labelPoint, bendPoint, anchorPoint, radialPoint, isRerouted }) => {
      ...
      const lineProps: CurveProps = {
        ...
        index,
        points: isRerouted ? [anchorPoint, radialPoint, bendPoint, labelPoint] : [anchorPoint, labelPoint],
        key: 'line',
      };
      ...
      return (
        <ZIndexLayer
          ...
          key={`label-${entry.startAngle}-${entry.endAngle}-${entry.midAngle}-${index}`}
        >
          <Layer>
